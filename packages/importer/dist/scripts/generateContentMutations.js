...
mutation ${global.config.options.mode}${name} {
  ${global.config.options.mode}${name}(
    ...
    ? `localizations: {
              create: [
                ${localizedFields.filter(({ locale }) => locale !== global.config.defaultLocale).map(({ locale, fields }) => `
...
`;
const parseFields = (fields, enumerations) => Object.entries(fields)
    .filter(([key, value]) => {
    if (global.config.options.exclude.field[key])
        return false;
...
const generateContentMutations = (data) => {
    ...
    const contentMutations = data.reduce((parsedModels, model) => {
        ...
        if (global.config.options.exclude.model[name])
            return parsedModels;
