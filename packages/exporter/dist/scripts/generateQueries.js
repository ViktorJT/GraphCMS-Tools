...
const triage = (apiId, isSystem) => {
    if (apiId === 'id')
        return true;
    if (global.config.exclude.field[apiId])
        return false;
    if (global.config.exclude.model?.[apiId])
        return false;
    if (global.config.include.includeSystemFields === false && isSystem)
        return false;
    return true;
};
const assertLocalization = (fields, modelIsLocalized) => modelIsLocalized
    ...
    : [fields, null];
const assertFieldSubType = (apiId, type) => {
    if (type && global.config.exclude.subType[type])
        return ' ';
    switch (type) {
        case 'RICHTEXT':
            return `${apiId} { raw }`;
...
const assertFieldType = ({ apiId, __typename, ...field }) => {
    if (global.config.exclude.type?.[__typename])
        return ' ';
    switch (__typename) {
        case 'SimpleField':
...
const parseFields = (fields) => fields.reduce((parsedFields, field) => triage(field.apiId, field.isSystem)
    ? [...parsedFields, assertFieldType(field)]
    : parsedFields, []);
const generateQueries = (schema) => {
    const contentQueries = schema.reduce((parsedQueries, { apiId: modelApiId, apiIdPlural: modelApiIdPlural, isLocalized: modelIsLocalized, fields: modelFields, }) => {
        ...
        return triage(modelApiId)
          ...
          ${modelApiId}: ${lowerCaseFirstLetter(modelApiIdPlural)}(
            ...
            ${modelIsLocalized
                    ? `localizations(includeCurrent: true ${global.config.targetLocales.length
